...
def grpo_train(
    ...
    for update in range(n_updates):
        ...
        for (traj, _, _), adv in zip(rollouts, advantages):
            ...
            log_std_reg = 0.01 * policy.log_std.pow(2).sum()
            loss = (pg + beta * kl + log_std_reg) / float(G)
            loss.backward()
        ...
        mean_reward = float(rewards.mean())
        if (sr > best_sr) or (sr == best_sr and mean_reward > best_reward):
            best_sr = sr
            best_reward = mean_reward
            torch.save(policy.state_dict(), checkpoint_path)
            print(f"  ✓ New best SR={sr:.2f}, r={mean_reward:.2f} — checkpoint saved")
...
def main():
    ...
    p.add_argument("--init_policy_ckpt", default=None,
                   help="Optional policy state_dict checkpoint to resume from.")
    p.add_argument("--checkpoint_path", default="models/raid_visual_grpo_best.pt")
    p.add_argument("--log_path", default="configs/grpo_libero_log.json")
    args = p.parse_args()
    ...
    if args.init_policy_ckpt:
        init_state = torch.load(args.init_policy_ckpt, map_location="cpu", weights_only=False)
        ...
        print(f"Loaded init policy from {args.init_policy_ckpt}")
